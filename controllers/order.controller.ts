...
export class OrderController {
  ...
  static async createOrder(req: Request, res: Response): Promise<Response> {
    try {
      ...
      let restaurantId = req.user?.restaurantId || req.user?.id;

      // If user is a customer (who has no role property in JWT token), extract restaurantId from body
      if (!req.user?.role && req.body.restaurantId) {
