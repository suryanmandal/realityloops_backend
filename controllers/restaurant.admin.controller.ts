...
export class RestaurantAdminController {
  ...
  /**
   * @route   GET /api/v1/admin/restaurant/dashboard/stats
   * @desc    Get global admin dashboard statistics
   * @access  Private (Admin only)
   */
  static async getDashboardStats(req: Request, res: Response): Promise<Response> {
    try {
      const result = await RestaurantAdminService.getAdminDashboardStats();
      return res.status(200).json(result);
    } catch (error: any) {
      logger.error("Get admin dashboard stats controller error", {
        error: error.message,
      });
      return res.status(500).json({
        success: false,
